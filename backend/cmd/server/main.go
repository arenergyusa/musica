...
func main() {
	...
	api := router.Group("/api/v1")
	{
		...
		admin.Use(middleware.AuthMiddleware(dbPool, redisClient), middleware.AdminMiddleware())
		{
			...
			admin.GET("/transactions", adminH.GetAllTransactions)
			admin.GET("/salary/qualifications", adminH.GetSalaryQualifications)
			admin.GET("/salary/payout-logs", adminH.GetSalaryPayoutLogs)
