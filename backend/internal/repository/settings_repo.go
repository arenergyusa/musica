...
func (r *settingsRepository) GetSettings(ctx context.Context) (*domain.PlatformSettings, error) {
	query := `
		SELECT 
			...
			invite_reward_l1_pct, invite_reward_l2_pct, invite_reward_l3_pct,
	...
	err := r.db.QueryRow(ctx, query).Scan(
		...
		&s.InviteRewardL1Pct, &s.InviteRewardL2Pct, &s.InviteRewardL3Pct,
...
func (r *settingsRepository) UpdateSettings(ctx context.Context, s *domain.PlatformSettings) error {
	query := `
		UPDATE platform_settings SET
			...
			invite_reward_l1_pct = $10, invite_reward_l2_pct = $11, invite_reward_l3_pct = $12,
