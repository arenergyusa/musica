...
export default function WalletPage() {
  ...
  const handleExportCSV = () => {
    let csvContent = "Date,Status,Requested Amount,Fee Amount,Net Amount\n";
    withdrawals.forEach((w) => {
      const date = new Date(w.created_at || "").toLocaleDateString();
      csvContent += `"${date}","${w.status}",${w.amount_requested || w.amount || 0},${w.tds_amount || 0},${w.net_amount || 0}\n`;
    });
    const fileName = `musica_withdrawals_${new Date().toISOString().slice(0, 10)}.csv`;
    ...
    link.click();
    toast.success("CSV Withdrawal history downloaded successfully!");
  };
      ...
      <Card className="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-xl shadow-sm">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <CardDescription className="text-xs text-slate-500">View and export your withdrawal requests.</CardDescription>
          </div>
          <div className="flex flex-wrap items-center gap-2.5">
            <p className="text-sm text-slate-500">Showing all withdrawal history.</p>
            ...
            <Table>
              <TableHeader className="bg-slate-50 dark:bg-slate-800/50">
                <TableRow>
                  ...
                  <TableHead className="text-right text-xs font-bold text-slate-700 dark:text-slate-300">Fee</TableHead>
